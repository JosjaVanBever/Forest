...
def load_header(filename):
    '''Read the header from a Molpro 2012 FCIDUMP file.
...
def load_fcidump(filename):
    ...
    with open(filename) as f:
        ...
        # make temporary files to store the large two_mo array
        tmp_dir = 'tmp' + str(random.randint(0,999999))
        os.mkdir(tmp_dir)
        print('tmp_dir was: %s' % tmp_dir)
        datafile = path.join(tmp_dir, 'two_mo.dat')

        # initialize the integrals
        one_mo = np.zeros((nbasis, nbasis), dtype=np.longdouble)
        print('before two_mo')
        two_mo = np.memmap(datafile, dtype='longdouble', mode='w+', shape=(nbasis,nbasis,nbasis,nbasis))
        print('after two_mo')
        # two_mo = np.zeros((nbasis, nbasis, nbasis, nbasis), dtype=np.longdouble)
        core_energy = 0.0
...
def dump_header(filename, data):
    ...
    with open(filename, 'w') as f:
        ...
        # Write header
        print >> f, '&FCI\nNORB=%i,\nNELEC=%i,\nMS2=%i,\nUHF=%s,' % (norb, nelec, ms2, uhf)
        print >> f, 'ORBSYM='+",".join(str(orbsym[i]) for i in xrange(norb))+","
        print >> f, '&END'
